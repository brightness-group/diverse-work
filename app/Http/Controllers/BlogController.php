...

{
    ...
    { 
        ...
        $query = Blog::where('lang', 'like', \App::getLocale());
        if (!is_null($request->get('search'))) {
            $query->where('heading', 'like', '%' . $request->get('search') .'%')->orWhere('content', 'like','%' . $request->get('search') . '%');
        }

        $data['blogs'] = $query->paginate(1);
